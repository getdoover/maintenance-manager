...
from .app_state import MaintenanceManagerState

log = logging.getLogger()

class MaintenanceManagerApplication(Application):
    config: MaintenanceManagerConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: MaintenanceManagerUI = None
        self.state: MaintenanceManagerState = None

    async def setup(self):
        self.ui = MaintenanceManagerUI()
        self.state = MaintenanceManagerState()
